#################################
#  lambda periodic_execution_checker
#################################

locals {
  lambda_name_periodic_execution_checker = format("%.64s", replace("${var.prefix}-periodic-execution-checker", "/[^a-zA-Z0-9_]+/", "-" ))
}

resource "aws_iam_role" "periodic_execution_checker" {
  name = format("%.64s", replace("${var.prefix}-${var.aws_region}-periodic-execution-checker", "/[^a-zA-Z0-9_]+/", "-" ))
...
resource "aws_iam_role_policy" "periodic_execution_checker" {
  name = aws_iam_role.periodic_execution_checker.name
  role = aws_iam_role.periodic_execution_checker.id
    ...
    Statement = concat([
      ...
      },
      {
        ...
        Resource = concat([
          ...
          "arn:aws:logs:${var.aws_region}:${var.aws_account_id}:log-group:/aws/lambda/${local.lambda_name_periodic_execution_checker}:*",
...
resource "aws_lambda_function" "periodic_execution_checker" {
  depends_on = [
    aws_iam_role_policy.periodic_execution_checker
  ]

  filename         = "${path.module}/lambdas/periodic-execution-checker.zip"
  function_name    = local.lambda_name_periodic_execution_checker
  role             = aws_iam_role.periodic_execution_checker.arn
